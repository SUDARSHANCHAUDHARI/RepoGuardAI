...
// src/severity-gate.ts
var FAILURE_THRESHOLDS = [...SEVERITIES, "none"];
var severityRank = new Map(
  SEVERITIES.map((severity, index) => [severity, index])
);
function parseFailureThreshold(value) {
  const normalized = value.toLowerCase();
  if (!FAILURE_THRESHOLDS.includes(normalized)) {
    throw new Error(
...
// src/action.ts
function buildAuditArgs(inputs) {
  if (!["full", "security", "api"].includes(inputs.scope)) {
...
function renderActionSummary(data) {
  const rows = SEVERITIES.map(
    (severity) => `| ${severity[0].toUpperCase()}${severity.slice(1)} | ${data.counts[severity]} |`
  );
  return [
...
function writeActionValue(file, name, value) {
  if (!file) return;
  if (value.includes("\n") || value.includes("\r")) {
...
function reportCounts(path) {
  if (!existsSync2(path)) return emptyCounts();
...
function runAction() {
  ...
  const cliPath = join2(actionRoot, "dist", "cli.js");
  const run = spawnSync(process.execPath, [cliPath, ...args], {
    stdio: "inherit",
    windowsHide: true
  });
  const { config } = loadConfig(target);
  const reportDirectory = join2(target, config.report.outputDirectory, "reports");
  const jsonReport = join2(reportDirectory, "audit-report.json");
  const sarifReport = join2(reportDirectory, "audit-report.sarif");
